...
## 不適你的
varible.df <- data.frame(variables_genes_bone,variables_metabolites_bone)
knitr::kable(varible.df)
...
lasso_sum <- glmnet(sum_interaction_scale, Response_filter$sum, family = "binomial", alpha = 1, lambda = 0.051)
...
lasso_count <- glmnet(count_interaction_scale, Response_filter$count, family = "poisson", alpha = 1, lambda = 0.23)
...
## ----liver2
knitr::kable(variables_liver)



## ----pca
load("lasso/Datas.RData")
pca_g <- prcomp(genes_filter, center = TRUE, scale=TRUE)
pve_g = (pca_g$sdev^2)/sum(pca_g$sdev^2) #* 100
plot(pve_g, type="o", ylab="Eigenvalue", xlab = "Principal Component", main='genes')
plot(cumsum(pve_g), type="o", ylab="Explained Variance ", xlab = "Principal Component", main='genes')

pca_m <- prcomp(metabolites_filter, center = TRUE, scale=TRUE)
pve_m = (pca_m$sdev^2)/sum(pca_m$sdev^2) #* 100
plot(pve_m, type="o", ylab="Eigenvalue", xlab = "Principal Component", main='metabolites')
plot(cumsum(pve_m), type="o", ylab="Explained Variance ", xlab = "Principal Component", main='metabolites')
...
load("lasso/Datas.RData")
load("lasso/interaction_transfer.Rdata")
...
##合併
bone.df <- data.frame(variables_bone)
bone_row <- data.frame(variables_bone = c(NA, NA, NA, NA))
bone.df <- rbind(bone.df, bone_row)

brain.df <- data.frame(variables_brain)
brain_row <- data.frame(variables_brain = c(NA, NA, NA))
brain.df <- rbind(brain.df, brain_row)

kidney.df <- data.frame(variables_kidney)
kidney_row <- data.frame(variables_kidney = NA)
kidney.df <- rbind(kidney.df, kidney_row)

liver.df <- data.frame(variables_liver)
lung.df <- data.frame(variables_lung)

binary.df <- data.frame(variables_binary)
binary_row <- data.frame(variables_binary = NA)
binary.df <- rbind(binary.df, binary_row)

count.df <- data.frame(variables_count)
count_row <- data.frame(variables_count = c(NA, NA))
count.df <- rbind(count.df, count_row)

variable.df <- data.frame(bone.df, brain.df, kidney.df, liver.df, lung.df, binary.df, count.df)
## ----lasso_variable
